...
if [ -z "${GRANULAR_TOKEN:-}" ] || [ -z "${OIDC_TOKEN:-}" ]; then
  echo "Both GRANULAR_TOKEN and OIDC_TOKEN must be set"
  exit 1
fi
...
echo "//registry.npmjs.org/:_authToken=${GRANULAR_TOKEN}" > /tmp/pkg-a/.npmrc
...
echo "//registry.npmjs.org/:_authToken=${OIDC_TOKEN}" > /tmp/pkg-b/.npmrc

echo "A: $(cat /tmp/pkg-a/index.js)"
echo "B: $(cat /tmp/pkg-b/index.js)"
echo "Mode: $MODE (delay: ${DELAY}s)"
echo "A auth: granular token"
echo "B auth: OIDC token"
echo ""
